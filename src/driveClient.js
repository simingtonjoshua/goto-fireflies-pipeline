...
// storage. Uses a single pre-authorized OAuth2 refresh token rather than a service
...
// files there. The refresh token was obtained via Google's OAuth 2.0 Playground on
// 2026-07-20/21, authorizing as jsimington@budgetblinds.com with the minimal,
// non-sensitive `drive.file` scope (this app can only see/manage files it creates
// itself - it cannot browse or read the rest of Joshua's Drive). The destination
// folder is named "Call Recordings"; Joshua can move it under "Budget Blinds Company
// File Center" (or anywhere else) in the Drive UI at any time without breaking this
// app's access, since drive.file grants are tied to the file's resource id, not its
...
module.exports = { uploadRecording, extForContentType };
