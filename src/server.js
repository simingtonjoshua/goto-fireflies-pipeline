...
async function processRecordingBatch(batch) {
    ...
    const withMeta = batch.find((id) => recordingMetadata.has(id));
        ...
        console.log(
                    `Recording batch ${JSON.stringify(batch)} - archiving only ${winner} (known metadata: ${!!withMeta}), skipping ${JSON.stringify(skipped)} as duplicate leg recordings of the same call.`
                );
...
// against this call's interaction so it can be included in the Heymarket note.
