...
// Reusable admin route (added 2026-07-23, per Joshua): manually replays the
// summarize -> transcript Doc -> Heymarket-post steps for one call, given its
// recordingId plus the call's known metadata. Exists for exactly the failure mode
// found on 2026-07-23 (outbound call to 916-704-0736): a fast run of back-to-back
// deploys of THIS pipeline's own code landed in the narrow window between that call
// ending and its 45-second quiet-period finalize timer (see interactions.js) firing.
// Render's deploy cutover replaces the old process with a new one once the new one is
// healthy - a clean handoff, not a crash - so nothing was ever logged as an error, but
// every in-memory timer on the old process (including that call's pending finalize)
// was lost with it. The call's recording and transcript had both already been fetched
// successfully and the recording archived to its Drive folder by that point; only the
// summarize+post step never got to run. This route replays that last step by hand
// using data already visible in the Render logs for the affected call (the recordingId
// GoTo assigned it, plus the metadata from its own call-state log lines), reusing the
// exact same openaiClient.summarizeInteraction / driveClient.createTranscriptDoc /
// heymarketClient.postPrivateNote pipeline finalizeInteraction() itself calls - nothing
// about the summary/Doc/note is built differently here. Not wired up to
// interactions.js's automatic finalize/dedup bookkeeping (recentlyFinalizedConversation
// SpaceIds etc.) since it's meant for exactly this rare, already-diagnosed case, not
// for routine use - existing safeguards elsewhere still apply to the automatic path.
app.post('/admin/reprocess-call', async (req, res) => {
        try {
                  const {
                              recordingId,
                              externalNumber,
          ...
          if (!recordingId || !externalNumber || !direction) {
                      return res.status(400).json({ error: 'recordingId, externalNumber, and direction are required' });
          }

          console.log(`Admin: reprocessing call for recording ${recordingId} (${externalNumber}, ${direction})...`);
                  const transcript = await fetchTranscript(recordingId);

          const legRecords = [{
                      conversationSpaceId: 'admin-reprocess',
                      legId: recordingId,
                      recordingId,
                      transcript,
                      direction,
          ...
          const topFolderId = process.env.GOOGLE_DRIVE_FOLDER_ID;
                  ...
                  const folder = folderId
                    ? { id: folderId, webViewLink: `https://drive.google.com/drive/folders/${folderId}` }
                              : await driveClient.getOrCreateCallFolder(topFolderId, folderName);

          const metaRows = [['Phone number', driveClient.formatPhoneForDisplay(externalNumber)]];
                  if (direction !== 'OUTBOUND' && externalName) {
                              metaRows.push(['Caller ID', externalName]);
                  }
                  metaRows.push(
                              ['Direction', direction || 'unknown'],
                              ['Team Member(s)', csrPath.join(' → ') || 'unknown'],
          ...
          await driveClient.createTranscriptDoc(folder.id, `${folderName} - Call Summary & Transcript`, docHtml);
                  console.log(`Admin: created transcript Doc for reprocessed call in folder ${folder.id}.`);

          const noteText = `${summaryText}\n\nCall recording & transcript: ${folder.webViewLink}`;
                  await heymarketClient.postPrivateNote(externalNumber, noteText, direction);
                  console.log(`Admin: posted Heymarket private note for reprocessed call (${externalNumber}).`);

          res.json({ ok: true, folderLink: folder.webViewLink, summary: summaryText });
        } catch (err) {
                  console.error('Admin: failed to reprocess call:', err);
                  res.status(500).json({ error: err.message });
        }
});

// Lightweight admin route (added 2026-07-23, per Joshua): posts an ALREADY-KNOWN
// summary straight to Heymarket, with no re-fetching, re-summarizing, or Doc creation.
// Exists alongside /admin/reprocess-call for the narrower case where a call's
// transcript Doc already exists in Drive (created normally by the automatic pipeline)
// but its Heymarket post was skipped or lost for some other reason - re-running
// /admin/reprocess-call for that case would create a redundant second Doc in the same
// folder. Found useful the same day as /admin/reprocess-call itself: a real call
// (916-216-5726) had its Heymarket post wrongly suppressed by the DUPLICATE-LEG dedup
// safety net (see interactions.js's DUPLICATE-LEG DEDUP FALSE POSITIVE FIX) even though
// its transcript Doc had already been created correctly - this route posts the summary
// that was already generated (visible in the Render logs) without touching Drive again.
app.post('/admin/post-note', async (req, res) => {
        try {
                  const { externalNumber, direction, summaryText, folderLink } = req.body || {};
                  if (!externalNumber || !summaryText) {
                              return res.status(400).json({ error: 'externalNumber and summaryText are required' });
                  }
                  const noteText = folderLink ? `${summaryText}\n\nCall recording & transcript: ${folderLink}` : summaryText;
                  await heymarketClient.postPrivateNote(externalNumber, noteText, direction);
                  console.log(`Admin: posted Heymarket private note for ${externalNumber} (no Doc created).`);
                  res.json({ ok: true });
        } catch (err) {
                  console.error('Admin: failed to post note:', err);
                  res.status(500).json({ error: err.message });
        }
});
