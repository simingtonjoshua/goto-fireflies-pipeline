...
// labeled by role, but per the team's own call-handling habits, whichever side speaks
// FIRST on an inbound call is the team member (they're trained to greet the customer
// first), while on an outbound call the SECOND side to speak is the team member (the
// first "utterance" is usually ringback/voicemail-system audio or the customer picking
// up) - confirmed consistent with the one live sample seen so far (channel 1 = voicemail
// system spoke first, channel 0 = the outbound team member spoke second). This is a
// heuristic, not a guarantee - Joshua's own words were "this isn't 100% always going to
// be accurate but should be close" - so inferChannelRoles() below is a best-effort label
// applied BEFORE handing the transcript to the model, rather than something the model is
// asked to guess blind.
//
...
// using the order-of-first-utterance + call-direction heuristic described in the header
// comment above. Returns {} (no labels) if there's only one channel with any speech at
// all - nothing to infer a role FROM in that case, so callers should fall back to
// generic "Channel N" labels rather than guessing.
...
{Customer's name if they stated it themselves in the conversation - otherwise their phone number, formatted like (916) 306-0800} located at {Address, City, State (always assume CA unless a different state was specifically mentioned), ZIP}
...
{Customer's name if they stated it themselves in the conversation - otherwise their phone number, formatted like (916) 306-0800} - {affected window(s) or room, as specifically as the call allows}
